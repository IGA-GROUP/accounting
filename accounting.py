...
class SecurityApp:
    ...
    def add_resident(self):
        ...
        if name:
            if name not in self.residents:
                ...
                admin_name = simpledialog.askstring("Добавить жильца", "Введите ваше имя (администратор):")
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                log_entry = f"{timestamp} - Жилец {name} добавлен администратором {admin_name}."
    ...
    def remove_resident(self):
        ...
        if selected:
            ...
            self.update_residents()
            admin_name = simpledialog.askstring("Удалить жильца", "Введите ваше имя (администратор):")
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            log_entry = f"{timestamp} - Жилец {name} удалён администратором {admin_name}."
            self.log.append(log_entry)
            self.save_log()  # Сохраняем в файл
    ...
    def mark_enter(self):
        ...
        if name in self.residents:
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            user_name = simpledialog.askstring("Отметить вход", "Введите ваше имя:")
            log_entry = f"{timestamp} - {name} вошёл. Отметил вход пользователь {user_name}."
            self.log.append(log_entry)
            self.save_log()  # Сохраняем в файл
    ...
    def mark_exit(self):
        ...
        if name in self.residents:
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            user_name = simpledialog.askstring("Отметить выход", "Введите ваше имя:")
            log_entry = f"{timestamp} - {name} вышел. Отметил выход пользователь {user_name}."
            self.log.append(log_entry)
            self.save_log()  # Сохраняем в файл
